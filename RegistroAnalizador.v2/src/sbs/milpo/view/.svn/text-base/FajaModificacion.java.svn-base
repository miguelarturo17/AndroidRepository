...
public class FajaModificacion extends GDActivity {
        ...
        registrar.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
        		boolean checkErrorDatos= false;
                	...
                	if(checkErrorDatos){
                		Toast.makeText(FajaModificacion.this, R.string.error_registro, Toast.LENGTH_SHORT).show();
                		
                	}else{
                		Lectura lect= getLecturaFaja();
                		
                		if(mLectura.updateLecturaFaja(lect)){
	                		new AlertDialog.Builder(FajaModificacion.this).setTitle(R.string.title_success).
	                    	setMessage(R.string.succes_update).show();
	                		retroceder();
                		}else Toast.makeText(FajaModificacion.this,R.string.error_operacion, Toast.LENGTH_SHORT).show();
    ...
    public void cargarRegistroFajaView(){
    	...
    	textPeso.setText(lectura.getPeso());
    	...
    	textFecha.setOnClickListener(new OnClickListener() {
        	@Override
			public void onClick(View v) {
        		Fecha.cargarFechaActual(FajaModificacion.this);
		        mYear = Integer.parseInt(Fecha.getFecha_año());
        		mMonth = Integer.parseInt(Fecha.getFecha_mes())-1;
        		mDay = Integer.parseInt(Fecha.getFecha_dia());
	...
	/**ACTUALIZA LA FECHA Y HORA QUE ESCOGEMOS EN EL PICKER*/
		...
		new DatePickerDialog.OnDateSetListener() {
			public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
				try {//validamos que la fecha no sea mayor que la actual
					...
					boolean esMenor= Fecha.isFechaMenor(FajaModificacion.this, dateAfter);
					if(esMenor){
						mYear = year;
						mMonth = monthOfYear;
						mDay = dayOfMonth;
    ...
    public Lectura getLecturaFaja(){
    	...
    	for (int i=0; i<eUnidad.length;i++){
	    	try {
	    		if(eUnidad[i].getCodigo_elementoQuimico().equals(Sistema.ELEMENTO_CU)){
	    			//lectura.detalleLecturaList[0]= new DetalleLectura();
			...
			try {
				if(eUnidad[i].getCodigo_elementoQuimico().equals(Sistema.ELEMENTO_AG)){
					//lectura.detalleLecturaList[1]= new DetalleLectura();
	    	...
	    	try {
	    		if(eUnidad[i].getCodigo_elementoQuimico().equals(Sistema.ELEMENTO_PB)){
	    			//lectura.detalleLecturaList[2]= new DetalleLectura();
	    	...
	    	try {
	    		if(eUnidad[i].getCodigo_elementoQuimico().equals(Sistema.ELEMENTO_ZN)){
	    			//lectura.detalleLecturaList[3]= new DetalleLectura();
	    	...
	    	try {
	    		if(eUnidad[i].getCodigo_elementoQuimico().equals(Sistema.ELEMENTO_FE)){
	    			//lectura.detalleLecturaList[4]= new DetalleLectura();
    	...
    	try {
    		lectura.setPeso(textPeso.getText().toString());
    	} catch (Exception e) {}
    ...
    private boolean validarDatosVaciosSalida(Lectura lecAnterior){
    	...
    	if(!textPeso.getText().toString().equals(lecAnterior.getPeso()))checkVacio=false;
    ...
    /*public boolean updateLecturaFaja(){
    }*/
