...
public class PerforacionClonacion extends GDActivity {
	...
	private static final int TIME_DIALOG_ID = 1,  DATE_DIALOG_ID = 0;
	...
	//private int TITLE_TECLADO=0, KEY_MAX_VALOR=0;
	//private String KEY_PREFIJO="";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        registrar.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
                ...
                if(checkErrorDatos){
                	Toast.makeText(PerforacionClonacion.this, R.string.error_registro, Toast.LENGTH_SHORT).show();
                	
                }else{
                	Lectura lect= getLecturaPerforacion();
                	
                	if(mLectura.saveLecturaPerforacion(lect)){
	                	new AlertDialog.Builder(PerforacionClonacion.this).setTitle(R.string.title_success).
	                	setMessage(R.string.succes_register).show();
	                	retroceder();
                	} else Toast.makeText(PerforacionClonacion.this,R.string.error_operacion, Toast.LENGTH_SHORT).show();
    ...
    /**SETEAMOS LA LECTURA A COPIAR ANTES DE INICIAR LA INTERFAZ*/
    ...
    public void cargarRegistroPerforacionView(){
        ...
        /*
        //cargamos el spinner de taladro
        if(!taladroStrings.equals(""))
        cargarSpinnerValorTaladro(taladroStrings, lectura);
        */
      ...
      ///manejamos la fecha
    	...
    	textFecha.setOnClickListener(new OnClickListener() {
        	@Override
			public void onClick(View v) {
        		Fecha.cargarFechaActual(PerforacionClonacion.this);
		        mYear = Integer.parseInt(Fecha.getFecha_año());
        		mMonth = Integer.parseInt(Fecha.getFecha_mes())-1;
        		mDay = Integer.parseInt(Fecha.getFecha_dia());
	...
	private DatePickerDialog.OnDateSetListener mDateSetListener =
		new DatePickerDialog.OnDateSetListener() {
			public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
				try {
					//validamos que la fecha no sea mayor que la actual
					Date dateAfter= new Date(year, monthOfYear, dayOfMonth, mHour, mMinute);
					boolean esMenor= Fecha.isFechaMenor(PerforacionClonacion.this, dateAfter);
					if(esMenor){
						mYear = year;
						mMonth = monthOfYear;
						mDay = dayOfMonth;
    ...
    public Lectura getLecturaPerforacion(){
		...
		try {
			lectura.setFechaCreacion(fechaCreacion);
			if(fechaModificacion.equals("")){
				lectura.setFechaModificacion(fechaCreacion);
				lectura.setFecha(fechaCreacion);
			}else{
				lectura.setFechaModificacion(fechaModificacion);
    ...
    private boolean validarDatosVaciosSalida(){
    	...
    	if(!textReconocimiento.getText().toString().equals(lectura.getValor()))checkVacio=false;
    	if(!textTaladro.getText().toString().equals(lectura.getNroTaladro()))checkVacio=false;
